...
export default async function handler(req: NextRequest) {
  ...
  if (
    !(
      process.env.UNLEASH_PROXY_SECRET &&
      token &&
      safeCompare(token || '', process.env.UNLEASH_PROXY_SECRET || '')
    )
  ) {
    return new Response(
      JSON.stringify({
        ...
        error: 'Unauthorized',
        message:
          'Token missing or does not match UNLEASH_PROXY_SECRET environment variable',
      }),
      {
        status: 401,
